...
def profile():
    ...
    wpm = db.session.query(func.avg(TypingResult.wpm)).filter(TypingResult.user_id == current_user.user_id).scalar()

    ##### Calculate Friends Count #####

    friends_query = Friendship.query.filter(Friendship.user_id == current_user.user_id)
    ...
    account_delta = datetime.now() - current_user.registration_time
    ...
    rank = db.session.query(leaderboard_query.c.rank).filter(leaderboard_query.c.user_id == current_user.user_id).scalar()
